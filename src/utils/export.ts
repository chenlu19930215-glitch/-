...
/**
 * Export as PNG image only (single page).
 *
...
export async function exportAsPng(
  element: HTMLElement,
  filename: string = DEFAULT_PNG_FILENAME,
): Promise<void> {
  if (!element) {
    throw new Error('导出失败：未找到要导出的内容区域')
  }
  ...
  try {
    ...
    throw new Error(
      `导出 PNG 失败：${err instanceof Error ? err.message : String(err)}`,
    )
  }
}

/**
 * Check if a DOM element reference is available and export-ready.
 */
export function canExport(ref: React.RefObject<HTMLElement | null>): boolean {
